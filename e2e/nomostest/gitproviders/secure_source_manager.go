...
// TODO: Create
// SSMReaderEmail returns the email of the google service account with
// permission to read from Cloud Source Registry.
func SSMReaderEmail() string {
	return fmt.Sprintf("test-ssm@%s.iam.gserviceaccount.com", *e2e.GCPProject)
}

// SSMClient is the client that interacts with Google Cloud Source Repository.
...
// newSSMClient instantiates a new CSR client.
...
func (c *SSMClient) CreateRepository(name string) (string, error) {
	...
	_, err = c.shell.ExecWithDebug("gcloud", "beta", "source-manager", "repos",
		...
		"--member", fmt.Sprintf("serviceAccount:%s", SSMReaderEmail()),
